...
**The Pilot Rule.** The archive's beacon travels. One lit dot rides the route line down the page as the reader scrolls, holds in each stop's ring while they keep going, then catches up to the next, and the stops behind it stay filled so the page shows distance covered. It is the third piece of motion with a voice on this site, after the divider buoy and the Editor's Pick light, and it is confined to `/archive/`: no other page gets a travelling marker.

It earns the exception by being the page's own subject. The archive is the only surface here that is genuinely a sequence, so a marker moving through that sequence is the content moving, not decoration laid over it. Three constraints keep it quiet. It is driven by scroll position and never by an animation of its own, so a still page is completely still. It eases toward its target rather than tracking the scroll exactly, because a marker pinned to the scroll snaps between stops and pulls the eye off the page. And it is the only lit thing on the archive: the fill is a state, so it takes the accent under The Beacon Rule rather than a colour of its own. Under `prefers-reduced-motion` the pilot is not rendered at all and the stops keep their static beacons, with the current issue still marked.
