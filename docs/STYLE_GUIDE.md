...
issue's frontmatter like any other section. The subscribe card is shared with the archive
index and is generated by the issue template, so it needs no frontmatter, no per-issue
decision and no review. Do not write it into an issue file. Explore carried the same card
until 2026-09-04 and no longer does.
...
- Place one always-visible directory-control group immediately above the app cards. The
  ...
  the label and the applied order cannot disagree; the control's name is carried in a
  screen-reader span before it, so the button still announces `Sort by A-Z`.
...
- Keep the controls visually connected to the grid with a 16px gap and use compact 48px
...
- **128px square is the target, and the ceiling is firm.** Anything larger is resized
  down, because the site renders icons at 48px and at most 80px, and oversized files were once
